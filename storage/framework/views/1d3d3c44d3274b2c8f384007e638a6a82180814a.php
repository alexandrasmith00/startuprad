<script src="<?php echo e(asset('js/lib/jquery.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery-migrate.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/bootstrap.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.ui.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jRespond.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/nav.accordion.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/hover.intent.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/hammerjs.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.hammer.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.fitvids.js')); ?>"></script>
<!--<script src="<?php echo e(asset('js/lib/scrollup.js')); ?>"></script>-->
<script src="<?php echo e(asset('js/lib/smoothscroll.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.slimscroll.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.syntaxhighlighter.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/velocity.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/smart-resize.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/autosize.min.js')); ?>"></script>

<script src="<?php echo e(asset('js/lib/icheck.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.switch.button.js')); ?>"></script>

<script src="<?php echo e(asset('js/lib/bootbox.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/sweetalert.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/select2.full.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/bootbox.js')); ?>"></script>

<!--Forms-->
<script src="<?php echo e(asset('js/lib/jquery.maskedinput.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.validate.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.form.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/j-forms.js')); ?>"></script>
<script src="<?php echo e(asset('js/lib/jquery.loadmask.js')); ?>"></script>
<script src="<?php echo e(asset('js/apps.js')); ?>"></script>


<script>
    

 
</script>

<?php echo $__env->make('js.comments', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>

<?php echo $__env->yieldContent('scripts'); ?>

